...
import { Button, Checkbox, TextInput, Accordion, AccordionItem } from 'carbon-components-react';
...
export class OrderSummaryItem extends Component {
  ...
  constructor(props) {
    ...
    this.filteredChildren = React.Children.map(props.children, (child) => {
      if (child.props['data-icon']) { //Verify it matches the slot we are looking for.
    ...
    return (
      <div className={classes} {...other}>
        <p className="infra--order-summary-item__overview">
          <span className="infra--order-summary-item__name">{itemIcon} {name}</span>
          <span className="infra--order-summary-item__cost">{cost}</span>
        </p>
        <div className="infra--order-summary-item__children">
          {children}
        </div>
      </div>
    );
...
export class OrderSummaryTotal extends Component {
  render() {
    const { text, cost, textNote, costNote, type, children, className, ...other } = this.props;

    return (
      <p className="infra--order-summary-total" data-type={type || 'total'}>
        <span className="infra--order-summary-total__text">{text} <span className="infra--order-summary-total__note">{textNote}</span></span>
        <span className="infra--order-summary-total__cost">{cost} <span className="infra--order-summary-total__note">{costNote}</span></span>
      </p>
    );
  }
}
  ...
  render() {
    ...
    return (
      <Accordion className="infra--order-summary-promo">
        <AccordionItem title="Add promo code" className="infra--order-summary-promo__dropdown">
          <TextInput
            id=""
